...
CALL_INFO "Make sure to install Docker Desktop for Windows"
fi
...
CALL_INFO "You first need to install Docker Desktop for Mac"
fi

# Install Docker

CALL_COMMAND "wget https://get.docker.com -O docker_install.sh"

wget https://get.docker.com -O docker_install.sh

CALL_COMMAND "chmod +x docker_install.sh"

chmod +x docker_install.sh

CALL_COMMAND "./docker_install.sh"
...
{
mkdir -p ${CIMITRA_SERVER_DIRECTORY}/certs
} 1> /dev/null 2> /dev/null

{
mkdir -p ${CIMITRA_SERVER_DIRECTORY}/branding/icons
} 1> /dev/null 2> /dev/null
...
curl -LJO https://raw.githubusercontent.com/cimitrasoftware/agent/master/cimitra_agent_install.sh -o ./ ; chmod +x ./cimitra_agent_install.sh ; ./cimitra_agent_install.sh ${CIMITRA_SERVER_ADDRESS} ${CIMITRA_SERVER_PORT} ${CIMITRA_SERVER_ADMIN_ACCOUNT} ${CIMITRA_SERVER_ADMIN_PASSWORD} ignoretty install_backup
...
curl -LJO https://raw.githubusercontent.com/cimitrasoftware/agent/master/cimitra_agent_install.sh -o ./ ; chmod +x ./cimitra_agent_install.sh ; ./cimitra_agent_install.sh ${CIMITRA_SERVER_ADDRESS} ${CIMITRA_SERVER_PORT} ${CIMITRA_SERVER_ADMIN_ACCOUNT} ${CIMITRA_SERVER_ADMIN_PASSWORD} systemd ignoretty install_backup
...
{
${SUDO_COMMAND} /var/opt/cimitra/api/server/create ${CIMITRA_SERVER_ADDRESS} ${CIMITRA_SERVER_PORT} ${CIMITRA_SERVER_ADMIN_ACCOUNT} ${CIMITRA_SERVER_ADMIN_PASSWORD} function=CHANGE_TO_CUSTOM_ADMIN_ID userId=admin@cimitra.com^ newUserId=${CUSTOM_ADMIN_ACCOUNT_ID}^
