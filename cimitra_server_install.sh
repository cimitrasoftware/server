...
function mesbox {
	...
	frowny="\xe2\x98\xb9\xef"
	skullbones="\xe2\x98\xa0\xef"
	topline="\nâ•”â•"
	if [ $hasSmiley == true ] ; then
		topline=$topline"â•â•â•"
	fi
	if [ $hasfrowny == true ] ; then
		topline=$topline"â•â•â•"
	fi
	for (( c=1; c<=${#message}; c++ ))
	do
	...
	echo -e $topline
	if [ $hasSmiley == true ] ; then
		echo -e "â•‘ $smiley $message â•‘"
	else
		if [ $hasfrowny == true ]; then
			echo -e "â•‘ $skullbones $message â•‘"
		else
			echo -e "â•‘ $message â•‘"
		fi
	fi
	if [ $hasSmiley == true ] ; then
		bottomline="â•šâ•â•â•â•"
	else
		if [ $hasfrowny == true ] ; then
			bottomline="â•šâ•â•â•â•"
		else
