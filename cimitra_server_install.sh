...
declare -i CUSTOM_SCRIPT_EXISTS=`echo $?`
	...
	CUSTOM_LINK_ENABLED="${CUSTOM_LINK_ENABLED_IN}"
	fi

fi

echo "CUSTOM_LINK_ENABLED = ${CUSTOM_LINK_ENABLED}"
sleep 10
...
if [ $CUSTOM_LINK_ENABLED -eq 1 ]
then
sudo /var/opt/cimitra/api/server/create ${CIMITRA_SERVER_ADDRESS} ${CIMITRA_SERVER_PORT} ${CIMITRA_SERVER_ADMIN_ACCOUNT} ${CIMITRA_SERVER_ADMIN_PASSWORD} function=CREATE_CUSTOM_LINK linkName=${CUSTOM_LINK_NAME}^ linkAddress=${CUSTOM_LINK_ADDRESS}^ linkDescription=${CUSTOM_LINK_DESCRIPTION}^
fi
