...
then
CIMITRA_SERVER_ADDRESS=`ip address show eth0 | grep inet | head -1 | awk '{printf $2}' | awk -F "/" '{printf $1}'`
else
CIMITRA_SERVER_ADDRESS=`ifconfig | grep .255 | grep -v 172. | grep -v 127.0.0.1 | awk -F "inet" '{printf $2}' | awk '{printf $1}'`
fi
...
curl -LJO https://raw.githubusercontent.com/cimitrasoftware/agent/master/cimitra_nix_agent_install.sh -o ./ ; chmod +x ./cimitra_nix_agent_install.sh ; ./cimitra_nix_agent_install.sh ${CIMITRA_SERVER_ADDRESS} ${CIMITRA_SERVER_PORT} ${CIMITRA_SERVER_ADMIN_ACCOUNT} ${CIMITRA_SERVER_ADMIN_PASSWORD}
...
curl -LJO https://raw.githubusercontent.com/cimitrasoftware/agent/master/cimitra_nix_agent_install.sh -o ./ ; chmod +x ./cimitra_nix_agent_install.sh ; ./cimitra_nix_agent_install.sh ${CIMITRA_SERVER_ADDRESS} ${CIMITRA_SERVER_PORT} ${CIMITRA_SERVER_ADMIN_ACCOUNT} ${CIMITRA_SERVER_ADMIN_PASSWORD} systemd
...
CALL_INFO "[ Cimitra Linux Server Install Script - Start ]"
...
CALL_INFO "[ Cimitra Linux Server Install Script - Finish ]"
